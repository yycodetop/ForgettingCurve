...
export default {
    ...
    setup(props, { emit }) {
        ...
        watch(() => props.recitationData, (newData) => {
            if (isWaitingForReciteData.value && newData.length > 0) {
                if (reciteConfig.value.studyMode === 'match') {
                    initMatchGame(newData);
                } else {
                    initRecitationSession(newData);
                }
        ...
        const fetchPhonetic = async (word) => {
            ...
            try {
                ...
                if (Array.isArray(data) && data.length > 0) {
                    const item = data[0];
                    return item.phonetic || (item.phonetics && item.phonetics.find(p => p.text)?.text) || '';
                }
            } catch (e) { console.log('Phonetic fetch error:', e); }
            return '';
        };
        ...
        const saveEdit = async () => {
            ...
            if (props.currentBook?.type === 'word') {
                const newPhonetic = await fetchPhonetic(editForm.value.word);
                if (newPhonetic) phonetic = newPhonetic;
            }
        ...
        const openReciteSetup = (mode) => {
            ...
            else reciteConfig.value.selectedBookIds = [];
            showReciteSetup.value = true;
        };
        const toggleBookSelection = (id) => {
            const idx = reciteConfig.value.selectedBookIds.indexOf(id);
            if (idx === -1) reciteConfig.value.selectedBookIds.push(id);
            else reciteConfig.value.selectedBookIds.splice(idx, 1);
        };
        const handleStartRecitation = () => {
            if (reciteConfig.value.selectedBookIds.length === 0) return alert("请至少选择一个单词本");
            isWaitingForReciteData.value = true;
        ...
        const initRecitationSession = (data) => {
            ...
            reciteQueue.value = queue;
            reciteIndex.value = 0;
            if (reciteConfig.value.mode === 'timed') {
                reciteTimeRemaining.value = reciteConfig.value.duration * 60;
                startTimer();
            } else { reciteTimeRemaining.value = 0; }
        ...
        const startTimer = () => {
            ...
            reciteTimer.value = setInterval(() => {
                reciteTimeRemaining.value--;
                if (reciteTimeRemaining.value <= 0) {
                    clearInterval(reciteTimer.value);
                    alert("时间到！学习结束。");
                    exitRecitation();
                }
        ...
        const resetWordState = () => {
            ...
            reciteErrorCount.value = 0;
            reciteInput.value = '';
            reciteStatus.value = 'neutral';
            showAnswer.value = false;
            
            if (reciteConfig.value.studyMode === 'memorize') {
                memorizeStage.value = 0; 
                showHintMeaning.value = true;
                showHintWord.value = true; 
                startMemorizeFlash();      
            } else if (reciteConfig.value.studyMode === 'dictate') {
                showHintMeaning.value = false;
                showHintWord.value = false;
                setTimeout(() => speak(currentReciteWord.value?.word), 300);
                focusInput();
            } else {
                showHintMeaning.value = true;
                showHintWord.value = false;
        ...
        const startMemorizeFlash = async () => {
            const currentSession = memorizeSessionId.value;
            flashCount.value = 0;
            for (let i = 1; i <= 5; i++) {
                if (memorizeSessionId.value !== currentSession || !isReciting.value) return;
                flashCount.value = i;
                await speak(currentReciteWord.value.word, 'en');
                if (memorizeSessionId.value !== currentSession || !isReciting.value) return;
                await new Promise(resolve => setTimeout(resolve, 1000));
            }
            if (memorizeSessionId.value === currentSession && isReciting.value) {
                memorizeStage.value = 1;
                reciteInput.value = '';
                focusInput();
            }
        };
        ...
        const handleMemorizeCheck = (inputVal, targetVal) => {
            if (inputVal === targetVal) {
                reciteStatus.value = 'correct'; 
                setTimeout(() => { reciteStatus.value = 'neutral'; }, 500); 
                if (memorizeStage.value === 1) {
                    memorizeStage.value = 2;
                    reciteInput.value = '';
                    showHintWord.value = false; 
                    const spelling = targetVal.split('').join(', ');
                    speakQueue([{ text: spelling, lang: 'en' }, { text: targetVal, lang: 'en' }]);
                } else if (memorizeStage.value === 2) {
                    memorizeStage.value = 3;
                    reciteInput.value = '';
                    speak(targetVal, 'en');
                } else if (memorizeStage.value === 3) {
                    memorizeStage.value = 4;
                    reciteStatus.value = 'correct';
                    stopSpeaking();
                    autoNextTimer.value = setTimeout(() => nextWord(), 2000);
                }
            } else {
                reciteStatus.value = 'wrong';
                reciteInput.value = '';
                logMistake(currentReciteWord.value); 
                if (memorizeStage.value > 1) {
                    alert("记忆模糊！正在回滚至第一阶段重塑记忆！");
                    stopSpeaking();
                    memorizeStage.value = 1;
                    showHintWord.value = true;
                    speak(targetVal, 'en');
        ...
        const checkReciteAnswer = () => {
            ...
            const inputVal = reciteInput.value.trim().toLowerCase();
            const targetVal = currentReciteWord.value.word.trim().toLowerCase();

            if (reciteConfig.value.studyMode === 'memorize') {
                if (memorizeStage.value === 0) return; 
                handleMemorizeCheck(inputVal, targetVal);
                return;
            }
            ...
            if (inputVal === targetVal) {
                reciteStatus.value = 'correct';
                stopSpeaking();
                if (reciteErrorCount.value > 0) addToRetryQueue();
                autoNextTimer.value = setTimeout(() => nextWord(), 2000);
            } else {
                reciteStatus.value = 'wrong';
                reciteErrorCount.value++;
                logMistake(currentReciteWord.value);
        ...
        const addToRetryQueue = () => {
            const retryItem = { ...currentReciteWord.value, id: Date.now() + Math.random() };
            reciteQueue.value.push(retryItem);
        };
        ...
        const nextWord = () => {
            if (reciteIndex.value < reciteQueue.value.length - 1) {
                ...
                if (confirm("学习完成！是否重新开始？")) {
                    if (reciteConfig.value.order === 'random') reciteQueue.value.sort(() => Math.random() - 0.5);
                    reciteIndex.value = 0;
                    resetWordState();
                } else { exitRecitation(); }
            }
        ...
        const smartFormatInput = () => {
            ...
            for (let i = 0; i < target.length; i++) {
                if (valIdx >= val.length) break; 
                if (target[i] === ' ') { newVal += ' '; if (val[valIdx] === ' ') valIdx++; } 
        ...
        const handleKeydown = (e) => {
            if (isReciting.value) {
                ...
                else if (e.key === 'Backspace') { inputRef.value?.focus(); }
                else if (e.key === 'Tab' && reciteConfig.value.studyMode === 'dictate') { e.preventDefault(); showHintMeaning.value = true; }
                else if ((e.ctrlKey || e.metaKey)) { e.preventDefault(); replayAudio(); }
            }
        };
        ...
        const initMatchGame = (data) => {
            let allData = [...data];
            if (reciteConfig.value.order === 'random') allData.sort(() => Math.random() - 0.5);
            matchTotalQueue.value = allData;
            matchResults.value = [];
            matchCurrentRound.value = 0;
            matchGameMode.value = 'playing';
            isMatchingGame.value = true;
            startMatchRound();
        };
        const startMatchRound = () => {
            ...
            if (setB.length > 0) matchCol3.value = setB.map(i => ({ ...i, type: 'word', col: 3, cardId: genCardId(3, i) }));
            else matchCol3.value = [];
            if (meaningsPart2.length > 0) matchCol4.value = meaningsPart2.map(i => ({ ...i, type: 'meaning', col: 4, cardId: genCardId(4, i) }));
            else matchCol4.value = [];
            matchPairs.value = {}; matchPairOrderMap.value = {}; matchNextOrder.value = 1; matchSelection.value = null; 
        };
        const handleMatchClick = (item) => {
            if (matchPairs.value[item.cardId]) {
                const partnerCardId = matchPairs.value[item.cardId];
                delete matchPairs.value[item.cardId];
                if (partnerCardId) delete matchPairs.value[partnerCardId];
                delete matchPairOrderMap.value[item.cardId];
                if (partnerCardId) delete matchPairOrderMap.value[partnerCardId];
                return;
            }
            if (item.type === 'word') speak(item.word, 'en'); 
            if (!matchSelection.value || matchSelection.value.type === item.type) {
                if (matchSelection.value && matchSelection.value.cardId === item.cardId) matchSelection.value = null;
                else matchSelection.value = item;
                return;
            }
            const sel = matchSelection.value;
            matchPairs.value[sel.cardId] = item.cardId; 
            matchPairs.value[item.cardId] = sel.cardId;
            matchPairOrderMap.value[sel.cardId] = matchNextOrder.value;
            matchPairOrderMap.value[item.cardId] = matchNextOrder.value;
        ...
        return {
            ...
            handleExport: () => emit('exportBook', props.currentBook?.id),
            handleDownload: () => emit('download'),
            filteredVocab, 
            ...
            isMatchingGame, matchGameMode, matchCurrentRound, matchTotalQueue,
            matchCol1, matchCol2, matchCol3, matchCol4, matchSelection,
            handleMatchClick, getPairOrder, isPaired, isSelected, submitMatchRound, exitMatchGame,
            matchCorrectList, matchWrongList,
            
            memorizeStage, flashCount,
            voices, selectedVoiceURI, ttsRate, isSpeaking, replayAudio,
    ...
    <div class="h-full flex gap-6">
        <div class="flex-[2] bg-white rounded-3xl shadow-sm border border-slate-100 flex overflow-hidden">
            <div class="w-16 bg-slate-50 border-r border-slate-100 flex flex-col items-center py-4 gap-3 shrink-0 h-full">
                <div class="flex-1 w-full overflow-y-auto custom-scrollbar flex flex-col items-center gap-3 px-1">
                    <div v-for="book in books" :key="book.id" @click="$emit('selectBook', book)" class="w-10 h-10 rounded-xl flex items-center justify-center cursor-pointer transition-all relative group shrink-0" :class="currentBook && currentBook.id === book.id ? 'bg-indigo-600 text-white shadow-lg shadow-indigo-200' : 'bg-white text-slate-400 hover:bg-white hover:text-indigo-500 border border-slate-200'"><i :class="[book.icon, 'text-sm']"></i></div>
                </div>
                <div class="mt-2 pt-2 border-t border-slate-200 w-full flex justify-center shrink-0">
                    <button @click="openCreateModal" class="w-10 h-10 rounded-xl border border-dashed border-slate-300 text-slate-400 hover:border-indigo-400 hover:text-indigo-500 flex items-center justify-center transition mt-1"><i class="fas fa-plus"></i></button>
                </div>
            </div>
                        ...
                        <div v-if="selectedMistakeDate && mistakeGroups[selectedMistakeDate]">
                            ...
                            <div class="flex flex-col gap-3">
                                <div v-for="item in mistakeGroups[selectedMistakeDate]" :key="item.id" class="p-4 rounded-xl border border-slate-100 shadow-sm hover:shadow-md transition-shadow bg-slate-50/50 group flex justify-between items-center">
                                    ...
                                    <div class="text-xs text-slate-300 font-mono bg-slate-100 px-2 py-1 rounded">Book: {{ item.bookId || 'Unknown' }}</div>
                                </div>
                            </div>
                        </div>
